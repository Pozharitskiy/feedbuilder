...
(async () => {
  try {
    ...
    try {
      ...
      console.log(
        `🔍 Found session tables: ${tables.map((t) => t.name).join(", ")}`
      );
      ...
      for (const tableName of possibleTables) {
        try {
          const sessions = db
            ...
            .all() as any[];
          console.log(
            `📊 Found ${sessions.length} sessions in table '${tableName}'`
          );
          ...
          for (const session of sessions) {
            try {
              ...
              console.log(
                `Session for ${session.shop || "unknown"}: token length = ${
                  token.length
                }`
              );
              ...
              if (token && token.length < 40) {
                console.warn(
                  `⚠️ Deleting invalid session for ${session.shop} (token length: ${token.length})`
                );
              ...
              console.warn(`⚠️ Could not parse session:`, (e as any).message);
            }
          }
        } catch (tableError) {
          // Table doesn't exist, try next one
          console.log(
            `ℹ️ Table ${tableName} not found or error:`,
            (tableError as any).message
          );
        }
      }
      db.close();
