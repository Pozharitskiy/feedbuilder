...
export function flattenProducts(payload) {
    const edges = payload?.edges ?? [];
    return edges.map((e) => {
        const n = e.node;
        const image = n.images?.edges?.[0]?.node?.url ?? null;
        const variants = (n.variants?.edges ?? []).map((ve) => ve.node);
        return {
            id: n.id,
            title: n.title,
            handle: n.handle,
...
function computeUnitPrice(price, weight, weightUnit) {
    if (!weight || weight <= 0)
        return null;
...
export function buildXmlFeed(shopDomain, products) {
    const root = create({ version: "1.0", encoding: "UTF-8" }).ele("offers");
    for (const p of products) {
        for (const v of p.variants) {
            const price = Number(v.price);
            const unitPrice = computeUnitPrice(price, v.weight, v.weightUnit || undefined);
            const o = root.ele("o", {
                id: v.id,
                price: price.toFixed(2),
                stock: Math.max(0, v.inventoryQuantity),
            });
            o.ele("name").txt(p.title + (v.title && v.title !== "Default Title" ? ` — ${v.title}` : ""));
            if (p.image)
                o.ele("img").txt(p.image);
