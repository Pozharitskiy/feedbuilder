...
(async () => {
    try {
        ...
        try {
            ...
            const sessions = db.prepare("SELECT * FROM shopify_sessions").all();
            console.log(`🔍 Found ${sessions.length} sessions in database`);
            for (const session of sessions) {
                try {
                    const sessionData = JSON.parse(session.session_data);
                    if (sessionData.accessToken && sessionData.accessToken.length < 40) {
                        console.warn(`⚠️ Deleting invalid session for ${session.shop} (token length: ${sessionData.accessToken.length})`);
                        db.prepare("DELETE FROM shopify_sessions WHERE id = ?").run(session.id);
                    }
                }
                catch (e) {
